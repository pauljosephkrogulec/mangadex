...
import { tagApi, handleResponse } from "@/lib/api";
import type { Tag, TagWrite } from "@/lib/types";

const EMPTY_FORM: TagWrite = { name: "", description: "", groupName: "", isPrimary: false };
...
export default function TagSection() {
  ...
  const [createForm, setCreateForm] = useState<TagWrite>(EMPTY_FORM);
  const [createError, setCreateError] = useState<string | null>(null);
  const [createLoading, setCreateLoading] = useState(false);

  const [editingId, setEditingId] = useState<string | null>(null);
  const [editForm, setEditForm] = useState<TagWrite>(EMPTY_FORM);
  ...
  useEffect(() => {
    async function fetch() {
      setLoading(true);
      setError(null);
      const result = await handleResponse(tagApi.list({ page }));
      if (result.success) {
  ...
  async function handleCreate(e: React.FormEvent) {
    e.preventDefault();
    if (!createForm.name.trim() || !createForm.groupName.trim()) return;
    setCreateLoading(true);
    setCreateError(null);
    const result = await handleResponse(tagApi.create(createForm));
    if (result.success) {
  ...
  function startEdit(item: Tag) {
    ...
    setEditForm({ name: item.name, description: item.description ?? "", groupName: item.groupName, isPrimary: item.isPrimary });
    setEditError(null);
  }

  async function handleSave(id: string) {
    /* v8 ignore next */
    if (!editForm.name.trim() || !editForm.groupName.trim()) return;
    setEditLoading(true);
    setEditError(null);
    const result = await handleResponse(tagApi.update(id, editForm));
    if (result.success) {
  ...
  async function handleDelete(id: string) {
    ...
    const result = await handleResponse(tagApi.delete(id));
    if (result.success) {
  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-lg font-semibold text-md-text-primary">Tags ({total})</h2>
      ...
      {showCreate && (
        <form onSubmit={handleCreate} className="mb-4 p-4 rounded-lg border border-md-border bg-md-surface space-y-3">
          <h3 className="text-sm font-semibold text-md-text-primary">New Tag</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <input
              ...
              placeholder="Group name (e.g. genre)"
              className="w-full bg-md-background border border-md-border text-md-text-primary text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-md-accent placeholder:text-md-text-secondary"
            />
            <input
              ...
              className="w-full bg-md-background border border-md-border text-md-text-primary text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-md-accent placeholder:text-md-text-secondary sm:col-span-2"
            />
            <label className="flex items-center gap-2 text-sm text-md-text-secondary cursor-pointer">
              <input
                type="checkbox"
                checked={createForm.isPrimary || false}
          ...
          <button
            type="submit"
            disabled={createLoading || !createForm.name.trim() || !createForm.groupName.trim()}
        ...
        <table className="w-full text-sm">
          <thead className="bg-md-surface text-md-text-secondary">
            <tr>
              ...
              <th className="text-left px-4 py-2 font-medium hidden md:table-cell">Primary</th>
          ...
          <tbody className="divide-y divide-md-border">
            ...
            {!loading && items.length === 0 && (
              <tr><td colSpan={4} className="px-4 py-4 text-center text-md-text-secondary text-sm">No tags yet.</td></tr>
            )}
            {items.map((item) =>
              editingId === item.id ? (
                <tr key={item.id} className="bg-md-surface/50">
                  ...
                  <td className="px-4 py-2 text-right space-x-2">
                    ...
                    <button
                      ...
                      disabled={editLoading || !editForm.name.trim() || !editForm.groupName.trim()}
                ...
                <tr key={item.id} className="hover:bg-md-surface/50 transition-colors">
                  ...
                  <td className="px-4 py-2 text-md-text-secondary hidden sm:table-cell">{item.groupName}</td>
                  <td className="px-4 py-2 text-md-text-secondary hidden md:table-cell">{item.isPrimary ? "Yes" : "No"}</td>
                  <td className="px-4 py-2 text-right space-x-2">
